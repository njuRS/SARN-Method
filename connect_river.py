...
env.extent="MAXOF"
arcpy.env.workspace = "E:\gra_stu\worlddemtest\sentinel2_Norway\mndwi\mndwi_connect.gdb"  # make sure it is not too long


DEM = "t3000_acc_arcticdem_10m1"  #the coordinate must be the same
RS_error = "T20_T100_MNDWI_T32VNN_20180727T104021_bandpass_gabor_cpo20" 
Raster_RS = "T20_T100_MNDWI_T32VNN_20180727T104021_bandpass_gabor_cpo20_1"
cellsize = 10                                       
delete_area = 300
...
RS = "clip_" + RS_error
arcpy.Intersect_analysis([RS_error,buffer_DEM_clip],RS,"ALL","0","LINE")
#(RS_error,buffer_DEM_clip,RS)
...
extract_Raster_RS  ="clip_" + Raster_RS
extract_Raster_RS1 = ExtractByMask(Raster_RS, buffer_DEM)
extract_Raster_RS1.save(extract_Raster_RS)
arcpy.CalculateStatistics_management(extract_Raster_RS)
arcpy.BuildPyramids_management(extract_Raster_RS)
print "finish clip RS"


##Get RS and DEM vector
cal_Raster_RS_DEM = Raster(ToRaster_buffer_DEM) + Raster(extract_Raster_RS)
con_cal_Raster_RS_DEM = Con(cal_Raster_RS_DEM>=3,3)


RS_Polygon = "RasterToPolygon" + RS
arcpy.RasterToPolygon_conversion(con_cal_Raster_RS_DEM,RS_Polygon,"SIMPLIFY")



layer3 = "delete_area" + RS_Polygon
arcpy.MakeFeatureLayer_management( RS_Polygon ,layer3 )
arcpy.SelectLayerByAttribute_management(layer3,"NEW_SELECTION",'"Shape_Area"<3000')
